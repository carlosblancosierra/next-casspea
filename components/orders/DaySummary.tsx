import React from 'react';
import { getDayTotals } from './ordersUtils';
import { Order } from '@/types/orders';
import { Product } from '@/types/products';

interface DaySummaryProps {
    dateOrders: Order[];
    products: Product[];
}

const DaySummary: React.FC<DaySummaryProps> = ({ dateOrders, products }) => {
    const { products: totals, flavors, randomBoxes } = getDayTotals(dateOrders, products);
    return (
        <div className="bg-main-bg dark:bg-main-bg-dark rounded-lg shadow-sm mt-4">
            <div className="px-4 py-5 sm:px-6">
                <h3 className="text-lg font-medium text-primary-text dark:text-primary-text-light">Day Summary</h3>
            </div>
            <div className="border-t border-gray-200 dark:border-gray-700">
                <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-gray-700">
                    {/* Products Column */}
                    <div className="p-4">
                        <h4 className="font-medium text-sm text-primary-text dark:text-primary-text-light mb-2">Products</h4>
                        <div className="space-y-1">
                            {Object.entries(totals).map(([name, qty]) => (
                                <div key={name} className="text-sm text-primary-text dark:text-primary-text-light">
                                    {name}: {qty}
                                </div>
                            ))}
                        </div>
                    </div>
                    {/* Pick & Mix Column */}
                    <div className="p-4">
                        <h4 className="font-medium text-sm text-primary-text dark:text-primary-text-light mb-2">Pick & Mix Flavours</h4>
                        <div className="space-y-1">
                            {Object.entries(flavors).length > 0 ?
                                Object.entries(flavors).map(([name, qty]) => (
                                    <div key={name} className="text-sm text-primary-text dark:text-primary-text-light">
                                        {name}: {qty}
                                    </div>
                                )) :
                                <div className="text-sm text-primary-text dark:text-primary-text-light">
                                    No Pick & Mix Flavours
                                </div>
                            }
                        </div>
                    </div>
                    {/* Random Boxes Column */}
                    <div className="p-4">
                        <h4 className="font-medium text-sm text-primary-text dark:text-primary-text-light mb-2">Other Flavours</h4>
                        <div className="space-y-1">
                            {Object.entries(randomBoxes).length > 0 ?
                                Object.entries(randomBoxes).map(([key, qty]) => (
                                    <div key={key} className="text-sm text-primary-text dark:text-primary-text-light">
                                        {key === 'Random' ? `Random: ${qty}` : `${key}: ${qty}`}
                                    </div>
                                )) :
                                <div className="text-sm text-primary-text dark:text-primary-text-light">
                                    No Random Boxes
                                </div>
                            }
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default DaySummary; 
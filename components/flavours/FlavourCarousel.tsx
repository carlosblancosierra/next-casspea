...
export default function FlavourCarousel() {
  ...
  return (
    <div
      ref={containerRef}
      className="flex overflow-x-hidden py-5 bg-white dark:bg-gray-900" // Background adjusted for dark mode
    >
      <motion.div
        ref={carouselRef}
        drag="x"
      ...
      >
        {Array(CAROUSEL_LENGTH)
          ...
          .map((_, index) => (
            <motion.div
              key={index}
              className="flex-shrink-0 w-56 h-96 flex flex-col items-center justify-center bg-white dark:bg-gray-900 text-black dark:text-white rounded-lg"

...
export default function StepFlavourSelection({
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-text-primary-text">Step 6: Choose Flavours</h2>
      ...
      {flavours.length > 0 && (
        <div className="space-y-2">
          {flavours.map((f, i) => (
            <div key={f.flavor.id} className="flex items-center p-2 border rounded">
              <Image
                ...
                className="rounded"
              />
              <span className="ml-3 flex-1 font-medium text-text-primary-text">{f.flavor.name}</span>
          ...
          <button
            onClick={onClear}
            className="text-sm text-text-primary-text hover:underline"
          >
            Clear all
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(f => (
          ...
          >
            ...
            <h3 className="mt-2 font-medium text-center text-text-primary-text">{f.name}</h3>
            <button
              onClick={() => onAdd(f)}
              ...
              className={`mt-3 px-4 py-2 rounded ${
                remaining > 0
                  ...
                  : 'bg-gray-300 text-text-primary-text cursor-not-allowed'
              }`}
            >
              + Add
            </button>
      ...
      <button
        onClick={onNext}
        disabled={remaining > 0}
        className="mt-6 w-full py-3 bg-blue-600 text-text-primary-text rounded disabled:opacity-50"
      >
        Next: Order Summary
      </button>
    </div>

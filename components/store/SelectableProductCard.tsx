...
export default function SelectableProductCard({ product, onSelect, price }: Props) {
  ...
  return (
    ...
    >
      ...
      {price === 0 ? (
        <>
          <p className="text-md font-medium text-text-primary-text mt-4">{product.name}</p>
          <p className="mt-1 text-sm text-text-primary-text">Included in your pack</p>
          <p className="mt-1 text-sm text-text-primary-text">{product.weight} g</p>
        </>
      ) : (
        <>
          <p className="text-md font-medium text-text-primary-text mt-4">￡ {price !== undefined ? price : product.base_price}</p>
          <div className="flex justify-between mt-1">
            <div>
              <h3 className="text-xs md:text-sm text-text-primary-text h-12">{product.name}</h3>
              <p className="mt-1 text-sm text-text-primary-text">{product.weight} g</p>
            </div>
          </div>
        </>

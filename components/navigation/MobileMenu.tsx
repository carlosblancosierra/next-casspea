...
import { CiShoppingCart, CiShoppingTag, CiShoppingBasket, CiHome, CiCircleQuestion } from "react-icons/ci";
...
const MobileMenu = () => {
    ...
    return (
        <div className={`fixed bottom-0 left-0 z-50 w-full h-16 bg-main-bg border-t border-gray-200 dark:bg-main-bg-dark dark:border-gray-600 md:hidden ${isScrolled ? 'bg-opacity-80 dark:bg-opacity-80' : 'bg-opacity-100 dark:bg-opacity-100'
            }`}>
            <div className="grid h-full max-w-lg grid-cols-5 mx-auto font-medium">
                {menuItems.map(({ name, href, icon: Icon }) => (
                    <Link
                        key={name}
                        href={href}
                        className={`inline-flex flex-col items-center justify-center px-3 hover: dark:hover:bg-gray-800 group ${isActive(href) ? 'text-blue-600 dark:text-blue-500' : ''
                            }`}
                    >
                        <Icon

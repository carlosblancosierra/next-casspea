        ...
        {TESTIMONIALS.map((t, i) => (
          <blockquote key={i} className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs text-gray-400">• {t.reviews} review{t.reviews>1?'s':''}</span>
              <span className="text-xs text-gray-400 ml-auto">{t.country}</span>
            </div>
            <div className="flex items-center gap-1 mb-1">
              {Array.from({ length: t.rating }).map((_, i) => <span key={i} className="text-yellow-400">★</span>)}
              <span className="text-xs text-gray-500 ml-2">Rated {t.rating} out of 5</span>
            </div>

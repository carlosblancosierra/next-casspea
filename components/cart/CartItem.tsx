    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-[1fr,3fr] gap-x-4 gap-y-1 border bg-main-bg p-4 rounded-lg shadow-sm dark:bg-gray-800 md:p-6">
        ...
        <div className="col-span-1 space-y-4">
          <a href="#" className="text-base font-medium hover:underline text-text-primary-text">
            {entry.product.name}
          </a>

          {customization && customization.selection_type && (
            <p className="flex flex-wrap items-center gap-2">
              <span className="rounded-md px-2 py-1 text-xs font-medium text-text-primary-text ring-1 ring-inset ring-gray-500/10">
              ...
              {'allergens' in customization && customization.allergens
                ? customization.allergens.map((a: Allergen) => (
                    <span
                      key={a.id}
                      className="rounded-md px-2 py-1 text-xs font-medium text-text-primary-text ring-1 ring-inset ring-gray-500/10"
                    >
                      {a.name} Free
                    </span>
                  ))
          ...
          <p className="text-base font-bold text-text-primary-text">£{entry.product.current_price}</p>
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              <FiMinus className="w-4 h-4 text-text-primary-text" />
            </button>
            ...
            >
              <FiPlus className="w-4 h-4 text-text-primary-text" />
            </button>
          </div>
          ...
          <div className='hidden md:block'>
            ...
            {entry.pack_customization && (
              <p className="flex flex-col gap-4 mt-4">
                {hotChocolate && (
                  <div>
                    <p className="text-xs text-text-primary-text italic">Hot Chocolate</p>
                    <span className="rounded-md px-2 py-1 text-xs font-medium text-text-primary-text">
                    {hotChocolate.name}
                    </span>
                  </div>
                )}
                {giftCard && (
                  <div>
                    <p className="text-xs text-text-primary-text italic">Gift Card</p>
                    <span className="rounded-md px-2 py-1 text-xs font-medium text-text-primary-text">
                      {giftCard.name}
                    </span>
                  </div>
                ...
                {bark && (
                  <div>
                    <p className="text-xs text-text-primary-text italic">Chocolate Bark</p>
                    <span className="rounded-md px-2 py-1 text-xs font-medium text-text-primary-text">
                      {bark.name}
                    </span>
                  </div>
            ...
            {customization?.selection_type === 'PICK_AND_MIX' && (
                <div className="hidden md:block md:col-start-2">
                  <button
                    ...
                    className="px-4 py-2 text-text-primary-text rounded-md text-sm border border-text-primary-text"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-main-bg rounded-lg shadow dark:bg-gray-700 max-w-md w-full">
            <div className="flex items-center justify-between p-4 border-b dark:border-gray-600">
              <h3 className="text-xl text-text-primary-text">Remove Item</h3>
              <button onClick={() => setIsDeleteModalOpen(false)} className="text-primary-text">
                <span className="sr-only">Close modal</span>×
              </button>
            </div>
            <div className="p-4 text-text-primary-text">
            ...
            <div className="flex justify-end p-4 border-t dark:border-gray-600">
              <button
                ...
                className="px-5 py-2.5 text-sm font-medium text-text-primary-text bg-red-600 rounded-lg"
              >
                Yes, remove
              </button>
              <button
